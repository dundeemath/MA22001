...
$$
\vv{r}(\rho, \phi) = [\rho \cos \phi,\,\rho\sin\phi].
$$
...
$$h_\rho = \left|\frac{\partial \vv{r}}{\partial \rho}\right| = 1,$$
...
# Spherical Polars
This one's a bit more complicated. We use the same $\phi$ for the angle around from the $x$-axis in the $x-y$ plane^[We call this the _azimuthal_ angle.] and now we introduce $\theta$ as the angle down from the $z$-axis^[We call this the _polar_ angle.]. Also, we use $r$ as the distance from the origin:
